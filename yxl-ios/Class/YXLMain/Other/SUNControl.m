...
- (void)closeButtonAction:(id)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(closeSUNAction)]) {
        [self.delegate closeSUNAction];
    }
}

- (void)finishInputAction:(id)sender {
    [self endEditing:YES];
}
...
- (UIImageView *) imageView {
    if (!_imageView) {
        ...
        _imageView.backgroundColor = SUN_GlobalTextGreyColor;
...
- (UIButton *) closeButton {
    if (!_closeButton) {
        ...
        _closeButton.backgroundColor = SUN_GlobalTextGreyColor;
...
- (SUNNumberChooseControl *) numberControl {
    if (!_numberControl) {
        _numberControl = [[SUNNumberChooseControl alloc] initWithFrame:CGRectMake(_padding, _padding, _contentSize.width - 2 * _padding, 56)];
    }
...
- (UIView *) keyboardView {
    if (!_keyboardView) {
        ...
        _keyboardView.backgroundColor = SUN_GlobalTextGreyColor;
